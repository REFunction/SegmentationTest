...
class VOC2012:
    def __init__(self, root_path='./VOC2012/', aug_path='SegmentationClassAug/', image_size=(224, 224),
        ...
        self.aug_path = self.root_path + aug_path
    ...
    def read_aug_names(self):
        ...
        self.aug_names = list(set(self.aug_names) - set(self.val_names))
        print('augmentation data:', len(self.aug_names))
    ...
    def gray_to_rgb(image):
        ...
        for h in range(height):
            for w in range(width):
                result[h][w] = VOC2012.index_to_rgb(image[h][w])
        return result
...
if __name__ == '__main__':
    ...
    for i in range(10):
        a, b = voc2012.get_batch_val(32)
        cv2.imshow('image', a[0])
        cv2.imshow('label', VOC2012.gray_to_rgb(b[0]))
        cv2.waitKey(0)
